...
def cache_root() -> str:
    ov = os.environ.get("FW_MIGRATION_CACHE_DIR")
    if ov:
        return ov
    r = _data_root()
    return join(r, "cache", "migration", "cache") if is_remote(r) else join(r, "migration-cache")


def output_root() -> str:
    ov = os.environ.get("FW_MIGRATION_OUTPUT_DIR")
    if ov:
        return ov
    r = _data_root()
    return join(r, "cache", "migration", "output") if is_remote(r) else join(r, "migration-output")
